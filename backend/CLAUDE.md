...
- **Persistent memory** (`core/memory.py`) — onboarding, "remember that…" commands, background extraction.
...
- **Conversation drives** — construct `JarvisAssistant()`, monkeypatch `_speak_confirmation` /
...
Each spawn is a **new** `jarvis-conversation` thread that the loop `.join()`s before continuing — so
...
3. Jarvis speaks the fixed greeting — or, in announcement mode, speaks the announcement instead (via
...
`MemoryManager` stores what Jarvis knows about the boss in `data/memory.json` (git-ignored — personal
...
1. **Onboarding** (`_run_onboarding`, gated by `memory.is_onboarding_complete()`) runs once, synchronously,
   inside `run()` — *before* `self._detector.start()` — so it needs no wake word and has no mic contention
...
Jarvis asks which). A **reply** resolves against `email_ctx["last_read"]` — the same referent "who sent
